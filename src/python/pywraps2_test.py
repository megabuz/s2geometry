...
class PyWrapS2TestCase(unittest.TestCase):
    ...
    def testS2PolygonGetAreaIsWrappedCorrectly(self):
        ...
        self.assertAlmostEqual(
            cell_union.ExactArea(), polygon.GetArea(), places=10)
    ...
    def testGetS2LatLngVertexIsWrappedCorrectly(self):
        ...
        self.assertEqual("51.500152,-0.126235",
                         first_vertex.ToStringInDegrees())
        second_vertex = loop.GetS2LatLngVertex(1)
        self.assertIsInstance(second_vertex, s2.S2LatLng)
        self.assertEqual("51.500153,-0.126235",
                         second_vertex.ToStringInDegrees())
    ...
    def testS2PolygonIntersectsWithPolyline(self):
        ...
        line = s2.S2Polyline()
        line.InitFromS2LatLngs([
            s2.S2LatLng.FromDegrees(51.5, -0.128),
    ...
    def testS2LatLngRectRegion(self):
        rect = s2.S2LatLngRect(
            s2.S2LatLng.FromDegrees(1.0, 2.0), s2.S2LatLng.FromDegrees(
                3.0, 4.0))
    ...
    def testS2CellRegion(self):
        cell = s2.S2Cell(
            s2.S2CellId(s2.S2LatLng.FromDegrees(3.0, 4.0)).parent(8))
    ...
    def testS2PolylineRegion(self):
        ...
        self.assertTrue(
            polyline.MayIntersect(
                s2.S2Cell(s2.S2LatLng.FromDegrees(0.0, 0.0))))
        self.assertFalse(
            polyline.MayIntersect(
                s2.S2Cell(s2.S2LatLng.FromDegrees(3.0, 4.0))))
    ...
    def testS2CellIdCenterSiTi(self):
        cell = s2.S2CellId.FromFacePosLevel(3, 0x12345678,
                                            s2.S2CellId.kMaxLevel)
    ...
    def tests2EatrthAngleConversion(self):
        self.assertEqual(s2.S2Earth.ToAngle(s2.S2Earth.Radius()).radians(), 1)
        self.assertEqual(
            s2.S2Earth.ToDistance(s2.S1Angle.Radians(2)).value(),
            2 * s2.S2Earth.RadiusMeters())
        self.assertEqual(s2.S2Earth.ToRadians(s2.S2Earth.Radius()), 1)
        self.assertAlmostEqual(
            s2.S2Earth.ToMeters(s2.S1Angle.Degrees(180)),
            s2.S2Earth.RadiusMeters() * math.pi)
        self.assertEqual(
            s2.S2Earth.ToKm(s2.S1Angle.Radians(0.5)),
            0.5 * s2.S2Earth.RadiusKm())
        self.assertEqual(
            s2.S2Earth.KmToRadians(s2.S2Earth.RadiusMeters() / 1000), 1)
        self.assertEqual(
            s2.S2Earth.RadiansToKm(0.5), 0.5 * s2.S2Earth.RadiusKm())
        self.assertEqual(
            s2.S2Earth.MetersToRadians(s2.S2Earth.RadiansToKm(0.3) * 1000),
            0.3)
        self.assertEqual(
            s2.S2Earth.RadiansToMeters(s2.S2Earth.KmToRadians(2.5)), 2500)

    def test2EatrthSolidAngleConversion(self):
        self.assertEqual(1,
                         s2.S2Earth.SquareKmToSteradians(
                             pow(s2.S2Earth.RadiusMeters() / 1000, 2)))
        self.assertAlmostEqual(
            pow(0.5 * s2.S2Earth.RadiusKm(), 2),
            s2.S2Earth.SteradiansToSquareKm(pow(0.5, 2)))
        self.assertAlmostEqual(
            pow(0.3, 2),
            s2.S2Earth.SquareMetersToSteradians(
                pow(s2.S2Earth.RadiansToKm(0.3) * 1000, 2)))
        self.assertAlmostEqual(
            pow(2500, 2),
            s2.S2Earth.SteradiansToSquareMeters(
                pow(s2.S2Earth.KmToRadians(2.5), 2)))

    def test2EatrthToLongitudeRadians(self):
        earth_radius = s2.Meters(s2.S2Earth.RadiusMeters())

        # At the equator, ToLongitudeRadians behaves exactly like ToRadians.
        self.assertEqual(s2.S2Earth.ToLongitudeRadians(earth_radius, 0), 1)

        # The closer we get to the poles, the more radians we need to go the same
        # distance.
        self.assertGreater(
            s2.S2Earth.ToLongitudeRadians(earth_radius, 0.5),
            s2.S2Earth.ToLongitudeRadians(earth_radius, 0.4))

        # At the poles, we should return 2PI radians instead of dividing by 0.
        self.assertEqual(
            s2.S2Earth.ToLongitudeRadians(earth_radius, math.pi / 2),
            math.pi * 2)
        ...
        self.assertEqual(
            s2.S2Earth.ToLongitudeRadians(earth_radius, math.pi / 2 - 1e-4),
            math.pi * 2)

    def test2EatrthGetInitialBearing(self):
        TestConfig = namedtuple('TestConfig', 'description, a, b, bearing')
        test_configs = [
            TestConfig("Westward on equator", s2.S2LatLng.FromDegrees(0, 50),
                       s2.S2LatLng.FromDegrees(0, 100),
                       s2.S1Angle.Degrees(90)),
            TestConfig("Eastward on equator", s2.S2LatLng.FromDegrees(0, 50),
                       s2.S2LatLng.FromDegrees(0, 0), s2.S1Angle.Degrees(-90)),
            TestConfig("Northward on meridian",
                       s2.S2LatLng.FromDegrees(16, 28),
                       s2.S2LatLng.FromDegrees(81, 28), s2.S1Angle.Degrees(0)),
            TestConfig("Southward on meridian", s2.S2LatLng.FromDegrees(
                24, 64), s2.S2LatLng.FromDegrees(-27, 64),
                       s2.S1Angle.Degrees(180)),
            TestConfig("Towards north pole", s2.S2LatLng.FromDegrees(12, 76),
                       s2.S2LatLng.FromDegrees(90, 50), s2.S1Angle.Degrees(0)),
            TestConfig("Towards south pole", s2.S2LatLng.FromDegrees(-35, 105),
                       s2.S2LatLng.FromDegrees(-90, -120),
                       s2.S1Angle.Degrees(180)),
            TestConfig("Spain to Japan",
                       s2.S2LatLng.FromDegrees(40.4379332, -3.749576),
                       s2.S2LatLng.FromDegrees(35.6733227, 139.6403486),
                       s2.S1Angle.Degrees(29.2)),
            TestConfig("Japan to Spain",
                       s2.S2LatLng.FromDegrees(35.6733227, 139.6403486),
                       s2.S2LatLng.FromDegrees(40.4379332, -3.749576),
                       s2.S1Angle.Degrees(-27.2)),
        ]
    ...
    def test2EatrthGetDistance(self):
        ...
        south = s2.S2Point(0, 0, -1)
        west = s2.S2Point(0, -1, 0)

        self.assertAlmostEquals(
            s2.Miles(s2.S2Earth.GetDistance(north, south)).value(),
            s2.Miles(math.pi * s2.S2Earth.Radius()).value())
        self.assertEqual(s2.S2Earth.GetDistanceKm(west, west), 0)
        self.assertAlmostEquals(
            s2.S2Earth.GetDistanceMeters(north, west),
            math.pi / 2 * s2.S2Earth.RadiusMeters())

        self.assertAlmostEquals(
            s2.Feet(
                s2.S2Earth.GetDistance(
                    s2.S2LatLng.FromDegrees(0, -90),
                    s2.S2LatLng.FromDegrees(-90, -38))).value(),
            s2.Feet(s2.S2Earth.GetDistance(west, south)).value())

        self.assertAlmostEquals(
            s2.S2Earth.GetDistanceKm(
                s2.S2LatLng.FromRadians(0, 0.6),
                s2.S2LatLng.FromRadians(0, -0.4)), s2.S2Earth.RadiusKm())

        self.assertAlmostEquals(
            s2.S2Earth.GetDistanceMeters(
                s2.S2LatLng.FromDegrees(80, 27),
                s2.S2LatLng.FromDegrees(55, -153)),
            1000 * s2.S2Earth.RadiusKm() * math.pi / 4)
